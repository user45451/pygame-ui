...
class TextBox(ui):
    def __init__(self, x, y, width, height, text=''):
        ...
        self.text = text
        # 使用原始字符串以避免转义序列问题
        self.font = pygame.font.Font('ui/SIMHEI.TTF', 36)
    ...
    def own_draw(self, screen):
        ...
        new_font = pygame.font.Font('ui/SIMHEI.TTF', new_font_size)
        for line in lines:
        ...
        current_position = 0
        # 考虑字体缩放后的光标位置计算
        scaled_font = pygame.font.Font('ui/SIMHEI.TTF', new_font_size)
    ...
    def set_text(self, text):
        self.text = text
    def drag_event(self, event): # 重载这个函数，如果这个文本框在root_ui之下，被点击后，会一直执行这个函数，直到点击其它ui
        ...
        if event.type == pygame.KEYDOWN and (event.mod & pygame.KMOD_CTRL) and event.key == pygame.K_v:
                pasted_data = pyperclip.paste()
                self.text = self.text[:self.cursor_position] + pasted_data + self.text[self.cursor_position:]
                self.cursor_position += len(pasted_data)
